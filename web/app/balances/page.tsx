...
export default function BalancesPage(){
  ...
  const fetchBalance = async () => {
    ...
    try{
      ...
      const res = await fetch(`/api/balance/${encodeURIComponent(accountId)}`, { headers });
      const text = await res.text();
      if(!res.ok){
        try { setError(JSON.parse(text)?.message || text || "Failed to fetch balance."); }
        catch { setError(text || "Failed to fetch balance."); }
        setResult(null);
      } else {
        setResult(text);
      }
